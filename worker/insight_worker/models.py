...
class DataPage(Base):
    ...
    embedding: Mapped[Optional[Vector(1536)]] = mapped_column(Vector(1536))
...
class Files(Base):
    ...
    status: Mapped[str] = mapped_column(
        Enum("uploading", "analyzing", "idle", name="file_status"),
        server_default=text("'uploading'::file_status"),
    )
...
class Prompts(Base):
    ...
    status: Mapped[str] = mapped_column(
        Enum("answering", "idle", name="prompt_status"),
        server_default=text("'answering'::prompt_status"),
    )
...
class Documents(Base):
    ...
    status: Mapped[str] = mapped_column(
        Enum("ingesting", "idle", name="document_status"),
        server_default=text("'idle'::document_status"),
    )
...
class Sources(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["private.files.id"],
            ondelete="CASCADE",
            name="sources_file_id_fkey",
        ),
        ForeignKeyConstraint(
            ["prompt_id"],
            ["private.prompts.id"],
            ondelete="CASCADE",
            name="sources_prompt_id_fkey",
        ),
