    ...
    with TemporaryDirectory() as dir:
        ...
        with Session(engine) as session:
            ...
            try:
                ...
                if not pdf_service.validate_pdf_mime_type(original_path):
                ...
                if not inode.to_page:
                    try:
                        inode.to_page = pdf_service.get_pdf_page_count(original_path)
                    except PdfError:
                        raise IngestException("corrupted_file")

                try:
                    # Repair and optimize the PDF
                    pdf_service.repair_pdf(original_path, repaired_path)
                    pdf_service.optimize_pdf(repaired_path, optimized_path)
                except Exception:
                    raise IngestException("corrupted_file")
                ...
                page_texts = pdf_service.extract_pdf_pages_text(optimized_path)
